...
class StudentWindow(QMainWindow):
    ...
    def obj_control(self, func, obj):
        state = obj.currentItem() is None
        if func.__name__ != "data_loader" and state:
            message_box("Önce seçim yapmalısınız!")
        elif (func.__name__ == "data_loader" or func.__name__ == "change_id_number") and not state:
            id_number = find_id_number(obj.currentItem().text())
            func(id_number)
        elif func.__name__ == "updater" and not state:
            self.generate_value(func)
        focus_item(self.ui.le_id_number)

    def generate_value(self, func):
        if self.ui.cmb_branch.currentText() != "":
            ...
            QMessageBox.warning(self, "UYARI !!!", "Öğrencinin bir branşı(dalı) olmak zorundadır.")
    ...
    def updater(self, **kwargs):
        ...
        if False not in control_list:
            # obj.setDate(QDate.fromString(info_list[number], "yyyy-MM-dd"))
    ...
    def data_loader(self, id_number):
        ...
        self.ui.date_starting_work.setDate(QDate.fromString(starting_date, "yyyy-MM-dd"))
    ...
    def clean_data(self):
        self.ui.lw_students.setCurrentItem(None)
